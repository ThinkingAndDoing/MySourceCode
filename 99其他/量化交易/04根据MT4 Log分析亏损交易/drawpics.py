...
def drawCurve(array, legend_=colname, label_="EURUSD"):
    ...
    if label_.find("-") != -1:
        array.plot(
            figsize=(12, 6), color="red", grid=True, legend=legend_, label=label_
        )
    else:
        array.plot(
            figsize=(12, 6), color="green", grid=True, legend=legend_, label=label_
        )


def getRangeY(df, curOrder):
    theMinY = min(
        float(df.loc[curOrder["Pendingtime"]]["<CLOSE>"]),
        float(df.loc[curOrder["Tradetime"]]["<CLOSE>"]),
        float(df.loc[curOrder["Closetime"]]["<CLOSE>"]),
    )
    theMaxY = max(
        float(df.loc[curOrder["Pendingtime"]]["<CLOSE>"]),
        float(df.loc[curOrder["Tradetime"]]["<CLOSE>"]),
...
def draw(df, curOrder):
    ...
    if curOrder["Tradetype"] == "buy":
        ...
        suffixOfLabel = "Buy"
        profit = round(
            df.loc[curOrder["Closetime"]]["<CLOSE>"]
            - df.loc[curOrder["Tradetime"]]["<CLOSE>"],
            5,
        )
    ...
    plt.scatter(
        list(df.index.values).index(curOrder["Pendingtime"]),
        df.loc[curOrder["Pendingtime"]]["<CLOSE>"],
        marker="o",
        c="",
        edgecolors=strEC,
    )
    plt.scatter(
        list(df.index.values).index(curOrder["Tradetime"]),
        df.loc[curOrder["Tradetime"]]["<CLOSE>"],
        marker="o",
        c="",
        edgecolors=strEC,
    ...
    plt.scatter(
        list(df.index.values).index(curOrder["Closetime"]),
        df.loc[curOrder["Closetime"]]["<CLOSE>"],
        marker="o",
        c="",
        edgecolors=strEC,
