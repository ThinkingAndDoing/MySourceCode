...
def countDiffIn60Secs():
    ...
    for i in range(1, len(GV._aDataBase)):
        fDiff = abs(
            Decimal(GV._aDataBase[i][dicCol["CLOSE"]])
            - Decimal(GV._aDataBase[i - 1][dicCol["CLOSE"]])
        )
        countDiff(fDiff)
        if (
            abs(
                int(GV._aDataBase[i][dicCol["TIME"]])
                - int(GV._aDataBase[i - 1][dicCol["TIME"]])
            )
            == 100
        ):
            if int(fDiff) > 6:
                print(
                    GV._aDataBase[i][dicCol["DTYYYYMMDD"]]
                    + ":"
                    + GV._aDataBase[i][dicCol["TIME"]]
                    + ":"
                    + GV._aDataBase[i][dicCol["CLOSE"]]
                    + ":"
                    + GV._aDataBase[i - 1][dicCol["CLOSE"]]
                    + "  Diff="
                    + str(fDiff)
                )
