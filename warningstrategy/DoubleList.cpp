...
bool WarningStrategy::insert_head(WarningView node)
{
	...
	{
		WarningView* new_head = new WarningView;   /*因为是新插入的节点，所以必须在堆中申请内存，否则函数执行完成后会自动释放内存*/
	...
	{
		WarningView* new_head = new WarningView;
...
bool WarningStrategy::insert_tail(WarningView node)
{
	...
	{
		WarningView* new_tail = new WarningView;
...
bool WarningStrategy::insertPriority(WarningView *pNode)
{
    ...
    while (p->Priority > pNode->Priority)       //若新报警pNode优先级低于p指向报警的优先级，继续向链表后面查找插入位置
    {
        if (NULL != p->next)
        {
            p = p->next;
...
bool WarningStrategy::AddNewWarningView(WarningView * pNewView)
{
    ...
    if (NULL == pCurrent)
    {
        ...
        if (pNewView->Priority >= pCurrent->Priority)
        {
            ...
            case WBDisplace:
                ...
                //当前报警不允许新来报警打断，新来报警加入m_newarrivallist队列
            case WBIgnore:
            	{
	            	NewArrival stNewArrivalTemp;
	                stNewArrivalTemp.Priority = pCurrent->Priority;
	                stNewArrivalTemp.WarningID = pCurrent->WarningID;
	                m_newarrivallist.push_back(stNewArrivalTemp); //此处是否有问题？局部变量在函数结束时会销毁，但此处应该只是值传递 
	                pCurrent->isPendingInterrupt = true;
				}
                break;
