...
senderList.append(
    {"host": "smtp.sina.com", "user": "xiewei23703@sina.com", "pwd": "***"}
)
...
def getStartDate():
    ...
    return str(
        datetime.date(localtime.tm_year - 10, localtime.tm_mon, localtime.tm_mday)
    )
...
def getEndDate():
    localtime = time.localtime(time.time())
    return str(
        datetime.date(localtime.tm_year - 9, localtime.tm_mon, localtime.tm_mday)
    )
...
def getLastTenYearsAnalyse():
    ...
    for i in range(0, 10):
        ...
        auPrice.setStartDate(
            getDate(localtime.tm_year - i - 1, localtime.tm_mon, localtime.tm_mday)
        )
        auPrice.setEndDate(
            getDate(localtime.tm_year - i, localtime.tm_mon, localtime.tm_mday)
        )
        auPrice.run()
...
def saveHistoryPriceToDB():
    ...
    auPrice = au99price.AuPrice()
    auPrice.setStartDate(
        getDate(localtime.tm_year - 10, localtime.tm_mon, localtime.tm_mday)
    )
    ...
    for item in auPrice.getHistoryRecods():
        insertSQL = (
            "INSERT INTO AUPRICE (DATE,CONTRACT,STARTPRICE,HIGHESTPRICE,LOWESTPRICE,ENDPRICE,FLUCTUATION,PRICECHANGERATIO,"
            + "WAP,TRADINGVOLUME,AMO) VALUES ('"
            + item[0]
            + "','"
            + item[1]
            + "','"
...
def getAuPrice():
    ...
    auPrice = au99price.AuPrice()
    auPrice.setStartDate(
        getDate(localtime.tm_year - 1, localtime.tm_mon, localtime.tm_mday)
    )
    auPrice.setEndDate(getDate(localtime.tm_year, localtime.tm_mon, localtime.tm_mday))
    auPrice.run()
    auParse = au99parse.AuParse()
