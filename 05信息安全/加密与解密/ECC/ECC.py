...
"""
ECC全称：椭圆曲线加密(Elliptic Curve Cryptography)，ECC加密算法是一种公钥加密技术，以椭圆曲线理论为基础。利用有限域上椭圆曲线的点构成的Abel群离散对数难解性，实现加密、解密和数字签名。将椭圆曲线中的加法运算与离散对数中的模乘运算相对应，就可以建立基于椭圆曲线的对应密码体制。 
...
这就是椭圆曲线加密算法的数学依据 
点G称为基点（base point），k（k<n）为私有密钥（privte key），K为公开密钥（public key) 
"""
