...
class debugger:
    ...
    def write_process_memory(self, address, data):
        ...
        if not kernel32.WriteProcessMemory(
            self.h_process, address, c_data, length, byref(count)
        ):
            return False
        else:
            return True
    ...
    def func_resolve(self, dll, function):
        error = None
        module = ctypes.WinDLL(dll, use_last_error=True)
        if not module._handle:
            print("Can't get the handle!")
            error = GetLastError()
        ...
        address = windll.kernel32.GetProcAddress(
            module._handle, function.encode(encoding="ascii")
        )
        if not address:
    ...
    def bp_set_mem(self, address, size):
        ...
        if kernel32.VirtualQueryEx(
            self.h_process, address, byref(mbi), sizeof(mbi)
        ) < sizeof(mbi):
            return False
