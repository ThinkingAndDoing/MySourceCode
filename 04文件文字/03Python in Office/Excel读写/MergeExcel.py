...
if __name__ == "__main__":
    notifDF = pd.DataFrame(
        pd.read_excel(
            "Notification Database DIM-E3 Freeze.xlsx",
            sheet_name="Notifications_GEEA2.0",
        )
    )
    notifDF = getNotifDF(notifDF)
    wrnlistDF = pd.DataFrame(
        pd.read_excel("TT&&WRN_List.xlsx", sheet_name="Text_Message")
    )
