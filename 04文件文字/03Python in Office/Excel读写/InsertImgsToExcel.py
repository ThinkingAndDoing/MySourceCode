...
def generateImgExcel(sourceSheet):
    ...
    for i in range(1, sourceSheet.nrows):
        lTable.append(
            [
                sourceSheet.cell(i, indexName).value,
                sourceSheet.cell(i, indexPIFLName).value,
                sourceSheet.cell(i, indexImage).value,
            ]
        )
    ...
    for i in range(0, len(srcTable)):
        ...
        if os.path.exists(strPath + srcTable[i][2] + ".png") == True:
            ...
            sheet.insert_image(
                i,
                2,
                strPath + srcTable[i][2] + ".png",
                {"x_scale": 1 / theScale, "y_scale": 1 / theScale},
            )
