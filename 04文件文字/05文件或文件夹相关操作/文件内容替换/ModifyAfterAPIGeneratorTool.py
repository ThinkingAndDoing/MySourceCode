...
def funcModify(fn, funcbody):
    if funcbody.find(fn) != -1:
        # print(fn)
        thefuncbody = re.search(
            r"" + fn + "[ ]*\([ ]*[\w]+[ ]*[\w_]+[ ]*\)[^\{]+{[^\}]+}", funcbody, re.I
        ).group()
...
def funcBodyModify(funcbody):
    if (
        funcbody.find("_u16Set") != -1
        or funcbody.find("_boSet") != -1
        or funcbody.find("_vSet") != -1
        or funcbody.find("_u8Set") != -1
        or funcbody.find("_u32Set") != -1
    ):
        newbody = ""
...
if __name__ == "__main__":
    ...
    allfunnames = (
        re.findall(vpattern, data)
        + re.findall(u16pattern, data)
        + re.findall(bopattern, data)
        + re.findall(u8pattern, data)
        + re.findall(u32pattern, data)
    )
    print(allfunnames)
    ...
    if os.path.exists(_BCTool):
        ...
        os.system(
            "BCompare.exe "
            + _CurPos
            + "\\pkg\\apisub\\pkg\\apimgr\\adapt\\API_Device_WDA "
            + _CurPos
            + "\\pkg\\artemmis\\adapt\\WDA_WidgetDummyApp\\core"
        )
        os.chdir(_CurPos)
    else:
