...
class MyTreeView:
    def __init__(self, root):
        ...
        # 放置水平方向滚动条
        hbar = ttk.Scrollbar(
            self.root, orient=tkinter.HORIZONTAL, command=self.treeView.xview
        )
    ...
    def createMyTree(self, mydict):
        myid = self.treeView.insert(
            "", 0, mydict[GV._RootKey]["name"], text=mydict[GV._RootKey]["name"]
        )
    ...
    for key in GV._Dict.keys():
        if GV._Dict[key]["parent"] in list(GV._Dict.keys()):
            if GV._Dict[key]["name"] not in GV._Dict[GV._Dict[key]["parent"]]["childs"]:
                GV._Dict[GV._Dict[key]["parent"]]["childs"].append(
                    GV._Dict[key]["name"]
                )
