...
def get_args():
	...
	parser.add_argument("-src", "--srcpath", help="absolute path to copy image from.", default=r".\\images")
	parser.add_argument("-dst", "--dstpath", help="absolute path to copy image to.", default=r".")
	parser.add_argument("-tt", "--timetype", help="rename by create time(ctime) or modify time(mtime).", default=r"ctime")
		...
		for filename in os.listdir(imgpath):
			...
			if os.path.isfile(fullPath):
				try:
					...
					strNewFileName = "["+str(tmCtime.tm_year) + "年" +str(tmCtime.tm_mon)+"月"+str(tmCtime.tm_mday)+"日"+str(tmCtime.tm_hour)+"时"+str(tmCtime.tm_min)+"分"+str(tmCtime.tm_sec)+"秒]"+ dImgDic["hash"][:6]+os.path.splitext(filename)[1]

					dImgDic["newfilename"] = os.path.join(self.dstImgPath, strNewFileName)
					shutil.copy(dImgDic["oldfilename"], dImgDic["newfilename"])
					
					print(dImgDic["oldfilename"]+" has been copied to "+dImgDic["newfilename"]+" !")
