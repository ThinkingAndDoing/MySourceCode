...
def copy():
	...
	if vari!="":
		...
		shutil.copytree(_CurPos+"\\tool\\Model",_CurPos+"\\variants\\"+vari+"\\hmi\\Model")

def compare():
	vari = getVariant()
	if os.path.exists(_BCTool):
		os.chdir(_BCTool)
		os.system("BCompare.exe "+_CurPos+"\\pkg\\artemmis "+_CurPos+"\\variants\\"+vari+"\\hmi\\artemmis")
		os.system("BCompare.exe "+_CurPos+"\\pkg\\apisub "+_CurPos+"\\variants\\"+vari+"\\hmi\\apisub")
		os.system("BCompare.exe "+_CurPos+"\\pkg\\gs4 "+_CurPos+"\\variants\\"+vari+"\\hmi\\gs4")
		os.chdir(_CurPos)
	else:
